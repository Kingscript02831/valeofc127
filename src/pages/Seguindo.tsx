...
export default function Seguindo() {
  ...
  useEffect(() => {
    const checkFollowingStatus = async () => {
      if (!currentUserId || !following || following.length === 0) {
        return;
      }
      
      const followingUserIds = following.map(f => f.profile?.id).filter(Boolean) as string[];
      
      if (followingUserIds.length === 0) {
        return;
      }

      const filteredUserIds = followingUserIds.filter(id => id !== currentUserId);
  ...
  const title = username ? `@${username}` : "Pessoas que você segue";
